...
export async function sendTelegramMessageToChat(text: string, chatId: string, retries = 3) {
  const token = process.env.TELEGRAM_BOT_TOKEN!;
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      if (response.ok) {
        console.log(`Telegram message sent to ${chatId} successfully on attempt ${attempt}`);
        return;
      } else {
        const errorText = await response.text();
        console.error(`Telegram API error on attempt ${attempt}: ${response.status} - ${errorText}`);
